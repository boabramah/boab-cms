...
class ApiContentController
{
    ...
    public function searchContentAction(Request $request, $format)
    {
        $searchTerm = $request->get('extra_search');
        $collection = $this->contentRepository->findContentBySearchTerm($searchTerm);
        $totalRecords = $this->contentRepository->findContentCount();
