...
class MenuWidgetListener implements EventSubscriberInterface
{
	...
	{
    	$view = $this->template->load('BoabCmsBundle:ToolBars:admin_authenticated_toolbar');
    	$view->user = $user;
    ...
    {
        $view = $this->template->load('BoabCmsBundle:ToolBars:unauthenticated_toolbar');
    ...
    private function createRouteView($menuItems=[])
    {
    	...
    	$view = $this->template->load('BoabCmsBundle:Widgets:sidebar_nav');
    	$view->menuItems = $menuItems;
    	$view->generateUrl = function($menu){
    		return $menu->getCleanUrl($this->router->generate('site_root'));
    	};
    	return $view->render();
    }
}